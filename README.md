...
## v0.3.0 — 存活检查与任务认领 (2026-06-30)

新增 4 项安全加固机制，彻底解决并行会话冲突问题：

1. **Worker Preamble** — 每轮执行前 5 项存活检查：task_plan 存在 → 定时器存活 → STOP 信号 → Claims 目录初始化 → Session 注册
2. **Task Claims** — `.plan/claims/{task-id}/` 锁机制，防止并行 session 重复执行同一任务
3. **STOP 信号** — `.plan/STOP` 标记文件，disable scheduled task 时同时写入，worker 启动即检查退出
4. **停止传播** — disable 必写 STOP，清理时 rm -f .plan/STOP

详见 [SKILL.md](SKILL.md) 的 Preamble、Claims、停止传播章节。
