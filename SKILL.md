...
15. **知识收割** → 提取失败/发现/模式写入 `.plan/knowledge/`
16. **更新 progress** → 当前任务标记 completed，写 checkpoint（含已用 token 估算），更新采纳率
17. **适应性退出** → 如果上下文余量 >30%，回到步骤 2 继续执行下一个 pending 任务；否则退出等待下次轮询
18. **全完成检查** → 所有任务 done → 清理退出
...
- **知识必收割**：每轮 batch 执行结束前必须执行知识收割，没有失败也要记录「无异常」
