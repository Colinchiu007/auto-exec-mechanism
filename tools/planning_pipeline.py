...
def enrich_gaps_with_graph(gaps: list[dict], context: dict):
    ...
    for hit in graph_hits:
        ...
        sem = " 📖" if hit["semantic_count"] > 0 else ""
...
def _generate_state_md(context: dict, gaps: list[dict], findings: list[dict]) -> str:
    """Generate STATE.md content (YAML frontmatter + markdown body)."""
    ...
    # Body
    yaml_lines.append("## Pipeline Summary")
    yaml_lines.append("")
    yaml_lines.append(f"- Goal: {context.get('goal', '(unspecified)')}")
    yaml_lines.append(f"- Scope: {context.get('scope', 'all')}")
    yaml_lines.append(f"- Total gaps: {total} (High: {effort_by_level.get('high', 0)}, Medium: {effort_by_level.get('medium', 0)}, Low: {effort_by_level.get('low', 0)})")
    yaml_lines.append(f"- Momus findings: {len(findings)}")
    yaml_lines.append("")
    yaml_lines.append("## Task List")
    yaml_lines.append("")

    if gaps:
        for i, g in enumerate(gaps, 1):
            priority = {"high": "P0", "medium": "P1", "low": "P2", "unknown": "P3"}.get(g.get("effort", "unknown"), "P3")
            yaml_lines.append(f"- [ ] **[{priority}]** {g['area']} --- {g['action'].split(chr(10))[0]}")
            yaml_lines.append(f"      Effort: {g['effort']} | Desired: {g['desired']}")
            yaml_lines.append("")
    else:
        yaml_lines.append("No tasks identified.")
        yaml_lines.append("")

    yaml_lines.append("---")
    yaml_lines.append(f"_Generated by planning_pipeline.py at {now}_")
...
Examples:
  ...
  %% python planning_pipeline.py --graph --goal "add video pipeline"
    ...
    if args.dry_run:
        print("\n" + plan)
        print("\n" + "\n".join(progress.split("\n")[:5]) + "\n...")
        state_md = _generate_state_md(context, gaps, findings)
        print("\n" + "\n".join(state_md.split("\n")[:10]) + "\n...")
        return
    ...
    # Write STATE.md (new canonical state format)
    state_md = _generate_state_md(context, gaps, findings)
    state_path = os.path.join(out_dir, "STATE.md")
    with open(state_path, "w", encoding="utf-8") as f:
        f.write(state_md)
    print(f"  -> STATE.md written ({os.path.getsize(state_path)} bytes)", file=sys.stderr)
